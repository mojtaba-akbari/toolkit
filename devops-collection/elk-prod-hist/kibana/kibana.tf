resource "helm_release" "Ekibana-hist" {
  name             = "kibana-hist"
  ...
  wait             = "false"
  depends_on       = [null_resource.kibana_community]
  values = [<<EOF
...
service:
  ...
  ##
  annotations: 
    cloud.networking.cafebazaar.cloud/load-balancer: zooket-logging
    cloud.networking.cafebazaar.cloud/load-balancer-external-ip: zooket-logging-lb
...
metrics:
  ...
  serviceMonitor:
    ...
    ##
    selector: {}
## @param elasticsearch [object] Properties for Elasticsearch
##
elasticsearch:
  tls: false
  ## e.g:
  ...
  ##
  hosts: 
    - elasticsearch-hist-master-0.elasticsearch-hist-master.logging.svc.cluster.local
  port: 9200
