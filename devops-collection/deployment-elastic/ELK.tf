resource "helm_release" "elk" {
  ...
  wait             = "false"
  depends_on       = [null_resource.elk_community]
  values = [<<EOF
...
master:
  ...
  persistence:
    ...
    ##   GKE, AWS & OpenStack)
    ##
    storageClass: "devspace-local-storage4"
...
data:
  ...
  persistence:
    ...
    ##   GKE, AWS & OpenStack)
    ##
    storageClass: "devspace-local-storage5"
